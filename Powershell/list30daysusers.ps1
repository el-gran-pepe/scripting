...
#Version con exclusion de usuarios
Get-ADUser -Filter * -Properties LastLogonDate, LockedOut, PasswordNeverExpires, CannotChangePassword, PasswordLastSet | 
    Where {
        $_.LastLogonDate -and 
        $_.LastLogonDate -le (Get-Date).AddDays(-30) -and 
        $_.Enabled -eq $true -and 
        $_.SamAccountName -notlike 'Usuario1' -and 
        $_.SamAccountName -notlike 'Usuario2'
    } | 
    Format-Table SAMAccountName, LastLogonDate, PasswordNeverExpires, CannotChangePassword -AutoSize | 
    Out-String

#esta nueva version es igual pero con exclusion de usuarios por nombre
